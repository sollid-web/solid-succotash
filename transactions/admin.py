...
from .models import Transaction, AdminAuditLog, CryptocurrencyWallet, VirtualCard, AdminNotification
from .services import approve_transaction, reject_transaction


@admin.register(AdminNotification)
class AdminNotificationAdmin(admin.ModelAdmin):
    list_display = ['title', 'notification_type', 'user', 'priority', 'is_read', 'is_resolved', 'created_at', 'action_links']
...
class VirtualCardAdmin(admin.ModelAdmin):
    list_display = ['user', 'card_type', 'masked_card_number', 'cardholder_name', 'balance', 'status', 'is_active', 'created_at']
    list_filter = ['card_type', 'status', 'is_active', 'created_at']
    search_fields = ['user__email', 'cardholder_name', 'card_number']
    readonly_fields = ['id', 'card_number', 'cvv', 'expiry_month', 'expiry_year', 'created_at', 'updated_at', 'expires_at']
    list_per_page = 25
    ...
    fieldsets = (
        ...
        ('Card Details', {
            'fields': ('card_number', 'cardholder_name', 'expiry_month', 'expiry_year', 'cvv', 'balance'),
            'classes': ('collapse',)
        }),
        ('Admin Fields', {
...
class CryptocurrencyWalletAdmin(admin.ModelAdmin):
    list_display = ['currency', 'display_wallet_address', 'network', 'is_active', 'updated_at']
    ...
    fieldsets = (
        (None, {
            'fields': ('currency', 'wallet_address', 'network', 'is_active')
        }),
        ('Timestamps', {
...
class TransactionAdmin(admin.ModelAdmin):
    list_display = ('user', 'tx_type', 'payment_method', 'amount', 'status', 'approved_by', 'created_at')
    list_filter = ('tx_type', 'payment_method', 'status', 'created_at', 'approved_by')
    search_fields = ('user__email', 'user__first_name', 'user__last_name', 'reference', 'tx_hash', 'id')
    readonly_fields = ('id', 'created_at', 'updated_at')
    date_hierarchy = 'created_at'
    actions = ['approve_transactions', 'reject_transactions']
    fieldsets = (
        (None, {
            'fields': ('id', 'user', 'tx_type', 'payment_method', 'amount', 'reference', 'status')
        }),
        ('Crypto Details', {
    ...
    def approve_transactions(self, request, queryset):
        ...
        for txn in queryset.filter(status='pending'):
            try:
                approve_transaction(txn, request.user, f"Approved via admin action by {request.user.email}")
                approved_count += 1
    ...
    def reject_transactions(self, request, queryset):
        ...
        for txn in queryset.filter(status='pending'):
            try:
                reject_transaction(txn, request.user, f"Rejected via admin action by {request.user.email}")
                rejected_count += 1
            except Exception as e:
                failed_count += 1
