...
from users.verification import issue_verification_token, verify_token, EmailVerification
...
def test_verification_flow():
    ...
    if result:
        print(f"   ✗ Invalid token was accepted (THIS IS BAD!)\n")
    else:
    ...
    result = verify_token(ev.token)
    if result:
        print(f"   ✗ Already-used token was accepted (THIS IS BAD!)\n")
    else:
        print(f"   ✓ Already-used token correctly rejected\n")
    ...
    print(f"   (Note: This will fail because token was already used above)")
