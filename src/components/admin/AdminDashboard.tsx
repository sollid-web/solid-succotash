...
type ResourceKey = 'overview' | 'api' | 'activity' | 'campaigns' | 'plans' | 'transactions' | 'investments' | 'kyc' | 'documents'
type Mode = 'list' | 'create' | 'edit' | 'show'
type FieldType = 'text' | 'textarea' | 'number' | 'datetime-local' | 'date' | 'url' | 'checkbox'

type ResourceConfig = {
  key: Exclude<ResourceKey, 'overview' | 'api' | 'activity'>
  label: string
  singular: string
  path: string
...
const resourceByKey = Object.fromEntries(resources.map((resource) => [resource.key, resource])) as Record<Exclude<ResourceKey, 'overview' | 'api' | 'activity'>, ResourceConfig>
...
export default function AdminDashboard() {
  ...
  async function onDelete(record: ApiRecord) { if (!window.confirm(`Delete ${String(record.title ?? record.name ?? record.id)}? This cannot be undone.`)) return; if (active === 'overview' || active === 'api' || active === 'activity') return; const resource = resourceByKey[active]; try { await deleteAdminResource(resource.path, record.id as string | number); setToast('Deleted successfully.'); setMode('list') } catch (caught) { setToast(caught instanceof Error ? caught.message : 'Delete failed.') } }
