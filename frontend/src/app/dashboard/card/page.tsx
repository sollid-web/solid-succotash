      ...
      <div className="max-w-sm w-full">
        ...
        <div className="rounded-2xl border border-blue-100 bg-white p-6 mb-5 shadow-sm">
          <div className="flex justify-between items-center py-3 border-b border-gray-100">
            ...
            <span className="text-sm font-semibold">Visa Virtual Card</span>
          </div>
          <div className="flex justify-between items-center py-3 border-b border-gray-100">
          ...
          <div className="flex justify-between items-center py-3 border-b border-gray-100">
            ...
            <span className="text-sm font-bold text-[#0b2f6b]">$1,000.00</span>
          </div>
          <div className="flex justify-between items-center py-3">
            <span className="text-sm text-gray-500">Processing Time</span>
            <span className="text-sm font-semibold">1–24 hours</span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold text-base shadow-lg hover:shadow-xl hover:brightness-105 transition-all disabled:opacity-60"
        >
    ...
    <div className="min-h-screen bg-[#F2F9FF] flex items-center justify-center px-4">
      <div className="max-w-sm w-full text-center">
        ...
        <p className="text-gray-500 text-sm leading-relaxed mb-8">
          Your virtual card request is being reviewed by our team. This usually takes 1–24 hours.
      ...
      <main className="max-w-lg mx-auto px-4 pt-6 pb-28">
        ...
        <div className="w-full cursor-pointer mb-2 select-none"
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontFamily: "monospace", fontSize: 15, letterSpacing: 3, color: "rgba(255,255,255,0.9)", textAlign: "center" }}>
                {displayNumber}
              </div>
            ...
            <div style={{
              ...
              <div style={{ padding: "0 24px" }}>
                ...
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", lineHeight: 1.6 }}>
                  Issued by WolvCapital for digital transactions only. Keep your CVV secure.
                </p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          {[
            ...
            { icon: isFrozen ? "🔥" : "❄️", label: freezing ? "Please wait…" : isFrozen ? "Unfreeze" : "Freeze Card",
              action: () => isFrozen ? handleFreeze() : setShowFreezeModal(true) },
            { icon: showCvv ? "🙈" : "👁️", label: showCvv ? "Hide CVV" : "Show CVV",
              action: () => { setShowCvv((v) => !v); if (!isFlipped) setIsFlipped(true); } },
            { icon: showFull ? "🔒" : "🔢", label: showFull ? "Hide Number" : "Show Number",
              action: () => setShowFull((v) => !v) },
          ].map((btn) => (
            <button key={btn.label} onClick={btn.action}
              disabled={freezing && btn.label.includes("reeze")}
        ...
        <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden mb-5">
          {[
            ...
            <button key={row.label}
              onClick={() => row.copy && copyToClipboard(row.copy, row.label)}
...
// ── MAIN PAGE — routes between the 3 states ───────────────
export default function VirtualCardPage() {
  ...
  // No card at all → request flow
  if (!card) return <RequestCardView onRequested={refetch} />;

  // Card exists but pending admin approval
  if (card.status === "pending" || card.status === "rejected") {
    return <PendingCardView onRefresh={refetch} />;
  }

  // Card approved and active (or suspended/frozen)
