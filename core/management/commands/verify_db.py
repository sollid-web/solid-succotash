...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if plan_count >= 4:
                self.stdout.write(self.style.SUCCESS(f"✅ Investment plans: {plan_count} plans available"))
            else:
                self.stdout.write(self.style.WARNING(f"⚠️ Investment plans: Only {plan_count} plans found (expected 4)"))
        except Exception as e:
        ...
        try:
            ...
            if wallet_count >= 4:
                self.stdout.write(self.style.SUCCESS(f"✅ Crypto wallets: {wallet_count} wallets ({', '.join(currencies)})"))
            else:
                self.stdout.write(self.style.WARNING(f"⚠️ Crypto wallets: Only {wallet_count} wallets found"))
        except Exception as e:
        ...
        try:
            ...
            if admin_users > 0:
                self.stdout.write(self.style.SUCCESS(f"✅ Admin users: {admin_users} admin(s) available"))
            else:
