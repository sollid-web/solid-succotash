...
class PublicPlansAPITests(TestCase):
	def setUp(self):
		...
		InvestmentPlan.objects.create(
			name='APIPlan', description='API', daily_roi=Decimal('1.00'), duration_days=14,
...
class UserInvestmentsAPITests(TestCase):
	def setUp(self):
		User = get_user_model()
		self.user = User.objects.create_user(username='api_user', email='api_user@example.com', password='pass12345')
		self.other = User.objects.create_user(username='api_other', email='api_other@example.com', password='pass12345')
		self.client = APIClient()
		self.plan = InvestmentPlan.objects.create(
			name='UserPlan', description='User', daily_roi=Decimal('1.00'), duration_days=14,
			min_amount=Decimal('100'), max_amount=Decimal('1000')
		)
...
class AdminInvestmentsAPITests(TestCase):
	def setUp(self):
		User = get_user_model()
		self.admin = User.objects.create_user(username='api_admin', email='api_admin@example.com', password='pass12345', is_staff=True)
		self.user = User.objects.create_user(username='api_norm', email='api_norm@example.com', password='pass12345')
		self.client = APIClient()
		plan = InvestmentPlan.objects.create(
			name='AdminPlan', description='Admin', daily_roi=Decimal('1.00'), duration_days=14,
			min_amount=Decimal('100'), max_amount=Decimal('1000')
		)
