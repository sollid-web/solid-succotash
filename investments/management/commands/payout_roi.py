...
from investments.models import UserInvestment, DailyRoiPayout
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--dry-run', action='store_true', help='Show what would be paid without creating transactions')
        parser.add_argument('--date', type=str, help='Process for a specific ISO date (YYYY-MM-DD)', default=None)

    def handle(self, *args, **options):
        process_date = timezone.now().date() if not options['date'] else timezone.datetime.fromisoformat(options['date']).date()
        dry = options['dry_run']
        paid = 0
        ...
        for inv in qs.select_related('plan', 'user'):
            ...
            if DailyRoiPayout.objects.filter(investment=inv, payout_date=process_date).exists():
                continue
            if dry:
                self.stdout.write(f"[DRY] Would pay {payout} to user {inv.user_id} for investment {inv.id}")
            else:
                with transaction.atomic():
                    ...
                    DailyRoiPayout.objects.create(investment=inv, payout_date=process_date, amount=payout)
                self.stdout.write(f"Paid {payout} to user {inv.user_id} (investment {inv.id})")
            paid += 1
            total_amount += payout
        self.stdout.write(self.style.SUCCESS(f"Processed {paid} investments. Total payout: {total_amount}"))
        if dry:
