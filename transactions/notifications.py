...
def get_unread_admin_notifications():
    ...
    notifications = AdminNotification.objects.filter(is_read=False, is_resolved=False).order_by(
        "created_at"
    )

    # Sort by priority manually
    sorted_notifications = sorted(
        notifications, key=lambda x: (priority_order.get(x.priority, 5), x.created_at)
    )
...
def get_admin_notification_count():
    ...
    return AdminNotification.objects.filter(is_read=False, is_resolved=False).count()
