...
type ResourceKey = 'overview' | 'api' | 'activity' | 'control' | 'campaigns' | 'plans' | 'transactions' | 'investments' | 'kyc' | 'documents'
type Mode = 'list' | 'create' | 'edit' | 'show'
type FieldType = 'text' | 'textarea' | 'number' | 'datetime-local' | 'date' | 'url' | 'checkbox'

type ResourceConfig = {
  key: Exclude<ResourceKey, 'overview' | 'api' | 'activity' | 'control'>
  label: string
  singular: string
  path: string
...
const resourceByKey = Object.fromEntries(resources.map((resource) => [resource.key, resource])) as Record<Exclude<ResourceKey, 'overview' | 'api' | 'activity' | 'control'>, ResourceConfig>
...
function LoginScreen({ onLogin }: { onLogin: (user: AdminUser) => void }) {
  ...
  return <main className="admin-console min-h-screen bg-[#f7f9fc] px-4 py-10 text-slate-950 sm:px-6 lg:px-8">
    <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl items-center justify-center">
      <div className="grid w-full overflow-hidden rounded-[2rem] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)] lg:grid-cols-[1.1fr_0.9fr]">
        <div className="admin-hero relative hidden min-h-[560px] overflow-hidden bg-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
...
function Sidebar({ active, onSelect, onLogout, user, onClose }: { active: ResourceKey; onSelect: (key: ResourceKey) => void; onLogout: () => void; user: AdminUser | null; onClose?: () => void }) {
  return <aside className="flex h-full w-[280px] shrink-0 flex-col bg-slate-950 text-white">
    ...
    <nav className="flex-1 px-3">
      ...
      <button onClick={() => { onSelect('overview'); onClose?.() }} className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === 'overview' ? 'bg-white text-slate-950' : 'text-slate-400 hover:bg-white/10 hover:text-white'}`}><LayoutDashboard size={17} /> Overview</button><button onClick={() => { onSelect('api'); onClose?.() }} className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === 'api' ? 'bg-white text-slate-950' : 'text-slate-400 hover:bg-white/10 hover:text-white'}`}><Braces size={17} /> API Center</button><button onClick={() => { onSelect('activity'); onClose?.() }} className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === 'activity' ? 'bg-white text-slate-950' : 'text-slate-400 hover:bg-white/10 hover:text-white'}`}><Users size={17} /> People & Presence</button><button onClick={() => { onSelect('control'); onClose?.() }} className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === 'control' ? 'bg-white text-slate-950' : 'text-slate-400 hover:bg-white/10 hover:text-white'}`}><ShieldCheck size={17} /> Django Control Center</button>
